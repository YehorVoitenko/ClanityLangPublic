...
from constants.exceptions import NotValidFileContentType, NotValidColumnSet, NotValidUserLimit
...
class FileValidator:
    ...
    def validate_content_type(file_content_type: str):
        if file_content_type not in AVAILABLE_FILE_FORMATS:
            raise NotValidFileContentType(f"The file must be in formats {AVAILABLE_FILE_FORMATS}.")
    ...
    def validate_data(cls, dataframe: DataFrame):
        ...
        for column_index in range(1, 3):
            ...
            raise ValueError(f"Column at index {column_index} contains nulls at rows: {list(null_indexes)}")
...
class QuizProcessor(FileValidator):
    ...
    async def process_user_new_file(cls, message: Message, state: FSMContext):
        ...
        await cls.create_file_in_storage_client(
            message=message,
            file=file,
            file_data=file_data_in_bytes
        )

    @classmethod
    async def process_xlsx_file(cls, file_data_in_bytes: bytes, file_content_type: str, message: Message,
                                state: FSMContext):
        try:
            await cls.validate_file_format(
                file_data_in_bytes=file_data_in_bytes,
    ...
    async def process_previous_user_file(cls, message: Message, state: FSMContext):
        ...
        try:
            file_response = storage_client.get_object(
                ...
                object_name=f"{message.chat.id}.xlsx"
            )
    ...
    async def process_file_with_words(file_data: BinaryIO, message: Message, state: FSMContext):
        try:
            ...
            limit = min(limit, len(word_pairs))
            word_pairs = list(word_pairs.items())

            random.shuffle(word_pairs)

            word_pairs = dict(word_pairs)
            word_pairs_by_limit = dict(list(word_pairs.items())[:limit])

            current_word = random.choice(list(word_pairs_by_limit.keys()))

            await state.set_data(
                {
                    StateKeys.QUIZ_DATA.value: word_pairs_by_limit,
                    StateKeys.CURRENT_WORD.value: current_word,
                    StateKeys.CURRENT_TRANSLATION.value: word_pairs_by_limit[current_word]
                }
            )
    ...
    async def start_quiz_with_b1_words(message: Message, state: FSMContext):
        ...
        if await PurchaseProcessor.compare_user_action_with_sub_level(
                message=message,
                need_sub_levels=need_sub_levels
        ):
    ...
    async def start_quiz_with_new_file(message: Message, state: FSMContext):
        ...
        if await PurchaseProcessor.compare_user_action_with_sub_level(
            ...
            await QuizProcessor.ask_user_to_send_file(
                message=message
            )
